...
> "No active applications to follow up on. Apply to some roles first with `/naymi-ai` and come back when they're aging."
...
- Suggest the user run `/naymi-ai contacto {company}` to find the right person first
...
> - Trying a different contact via `/naymi-ai contacto`
...
**To:** {email or "No contact found — run `/naymi-ai contacto` first"}
